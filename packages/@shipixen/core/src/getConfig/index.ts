...
import { fs, unknownToAbsolutePosixFilePath } from '@shipixen/utils'
import type { E } from '@shipixen/utils/effect'
import { Array, Chunk, O, OT, pipe, S, T } from '@shipixen/utils/effect'
...
export const getConfig = ({
  ...
  pipe(
    ...
    T.rightOrFail,
    OT.withSpan('@shipixen/core/getConfig:getConfig', { attributes: { configPath } }),
  )
...
const getConfigFromResult = ({
  ...
  pipe(
    ...
    OT.withSpan('@shipixen/core/getConfig:getConfigFromResult', { attributes: { configPath } }),
    T.either,
  )
