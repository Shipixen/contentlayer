...
import { filePathJoin, fs, relative } from '@shipixen/utils'
import type { E, HasClock, HasConsole } from '@shipixen/utils/effect'
import { Array, Chunk, Either, OT, pipe, S, T } from '@shipixen/utils/effect'
...
export const generateDotpkg = ({
  ...
  pipe(
    ...
    T.rightOrFail,
    OT.withSpan('@shipixen/core/generation:generateDotpkg', { attributes: { verbose } }),
  )
...
> =>
  pipe(
    T.gen(function* ($) {
      ...
      const individualDataJsFiles = allCacheItems.map(({ document, documentHash }) => ({
        content: `export default ${JSON.stringify(document, null, 2)}`,
        filePath: withPrefix('generated', document[typeNameField], `${idToFileName(document._id)}.js`),
        documentHash,
      }))

      const collectionDataJsFiles = pipe(
        documentDefs,
        Array.map((documentDef) => {
          const documents = allDocuments.filter((_) => _[typeNameField] === documentDef.name)
          const jsData = documentDef.isSingleton ? documents[0]! : documents

          return {
            content: `export default ${JSON.stringify(jsData, null, 2)}`,
            filePath: withPrefix('generated', documentDef.name, `_index.js`),
      ...
      yield* $(
        T.tuplePar(
          ...
          ...individualDataJsFiles.map(writeFile),
          ...collectionDataJsFiles.map(writeFile),
          options.experimental.enableDynamicBuild ? makeFetchContentWorker({ config, bundleFilePath }) : T.unit,
        ),
      )
    }),
    OT.withSpan('@shipixen/core/generation/generate-dotpkg:writeFilesForCache', {
      attributes: {
        targetPath,
...
const makeDataExportFile = ({
  ...
  if (docDef.isSingleton) {
    ...
    return `\
// ${autogeneratedNote}
export { default as ${dataVariableName} } from './${idToFileName(documentId)}.js'
`
  }
  ...
  const docImports = documentIds
    .map((_) => `import ${idToVariableNameMap.get(_)} from './${idToFileName(_)}.js'`)
    .join('\n')
  ...
  const constImportsForAllDocuments = dataVariableNames
    .map(({ documentDefName, dataVariableName }) =>
      isDev
        ...
        : `import ${dataVariableName} from './${documentDefName}/_index.js'`,
    )
    .join('\n')
...
import { SourceProvideSchemaErrorJSON, SourceFetchDataErrorJSON } from 'shipixen-contentlayer/core'
...
import { dynamicBuildMain } from '@shipixen/core'
  ...
  setup: (build) => {
    ...
    build.onResolve({ filter: /.+/ }, async ({ path, ...args }) => {
      ...
      if (path.match(filter)) {
        ...
        if (result.path.match(/\@shipixen\/[a-z-]+\/src\//)) {
