...
import { provideTracing } from '@shipixen/utils'
import { O, OT, pipe, provideConsole, S, T } from '@shipixen/utils/effect'
...
export const dynamicBuildFromCompiledConfig = ({
  ...
  pipe(
    ...
    OT.withSpan('@shipixen/core/dynamicBuildFromCompiledConfig', { attributes: { compiledConfigPath } }),
  )
...
export const dynamicBuild = ({ config, verbose }: { config: Config; verbose: boolean }) =>
  pipe(
    ...
    OT.withSpan('@shipixen/core/dynamicBuild'),
  )
