...
import { asMutableArray, fs, relativePosixFilePath } from '@shipixen/utils'
import type { HasConsole } from '@shipixen/utils/effect'
import { Chunk, O, OT, pipe, T } from '@shipixen/utils/effect'
...
> =>
  pipe(
    ...
    OT.withSpan('@shipixen/source-local/fetchData:fetchAllDocuments', { attributes: { contentDirPath } }),
  )
...
const getAllRelativeFilePaths = ({
  ...
  return pipe(
    ...
    OT.withSpan('@shipixen/source-local/fetchData:getAllRelativeFilePaths'),
  )
}
