...
import { identity, O, OT, pipe, T, These } from '@shipixen/utils/effect'
...
> =>
  pipe(
    ...
    OT.withSpan('@shipixen/source-local/fetchData:makeCacheItemFromFilePath', { attributes: { relativeFilePath } }),
    T.mapError((error) => {
      switch (error._tag) {
        case 'fs.StatError':
...
> =>
  pipe(
    ...
    OT.withSpan('@shipixen/source-local/fetchData:getRawContent'),
  )
